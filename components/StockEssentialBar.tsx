...
export function StockEssentialBar({
  ...
  return (
    <div className="rounded-xl border border-line bg-gradient-to-br from-gain-soft/20 to-white p-4">
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div className="min-w-0">
          <p className="text-2xl font-bold tabular-nums text-ink sm:text-3xl">{fmt(summary.currentPrice)}</p>
          {quote && (
        ...
        {recommendation && (
          <div className="text-right">
            ...
            <p className="text-xs text-ink-muted">{urgencyLabel(recommendation.urgency)}</p>
          </div>
        )}
      </div>
      ...
      <div className="mt-3 flex flex-wrap gap-2">
        ...
        {onRefreshPrice && (
          <button
            ...
            disabled={kisLoading}
            className="rounded-lg border border-line bg-white px-3 py-2 text-sm text-ink-muted hover:bg-surface-dim disabled:opacity-50"
          >
            {kisLoading ? "시세…" : "시세"}
          </button>
        )}
